 ...
 if ( ! function_exists( 'woocommerce_get_product_thumbnail' ) ) {
	...
	function woocommerce_get_product_thumbnail( $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
		...
		if ( has_post_thumbnail() ) {
			...
			$creativa_product_gallery = $product->get_gallery_attachment_ids();
			...
			$output .= '<img class="creativa_gallery_image_thumb '. creativa_retina_check($image_thumb[0], false) .'" src="'. $image_thumb[0] .'" width="'. $image_thumb[1] .'" height="'. $image_thumb[2] .'"  alt="" />';
			...
			if ( $creativa_product_gallery ) {
				...
				$output .= '<img class="creativa_gallery_image_thumb_overlay '. creativa_retina_check($image[0], false) .'" src="'. $image[0] .'" width="'. $image[1] .'" height="'. $image[2] .'"  alt="" />';
			}
			...
			if ( $creativa_product_gallery ) {
				$image_url = wp_get_attachment_url($creativa_product_gallery[0], 'full');
				$image = aq_resize( $image_url, $placeholder_sizes['width'], $placeholder_sizes['height'], $placeholder_sizes['crop'], false, true );
				$output .= '<img class="creativa_gallery_image_thumb '. creativa_retina_check($image[0], false) .'" src="'. $image[0] .'" width="'. $image[1] .'" height="'. $image[2] .'"  alt="" />';
			} else {
...
add_filter('add_to_cart_fragments', 'creativa_woo_header_add_to_cart_fragment');
...
function creativa_woo_header_add_to_cart_fragment( $fragments ) {
	...
	if ($icons_style == 1) {
		...
		?>
		<span class="nav__cart-subtotal"><?php echo esc_html($cart_subtotal); ?></span>
		<?php
	...
	elseif ($icons_style == 2 || $icons_style == 3) {
		...
		?>
		<span class="nav__cart-items"><?php echo esc_html($cart_items); ?></span>
		<?php
