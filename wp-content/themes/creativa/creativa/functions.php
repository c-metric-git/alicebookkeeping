...
if (class_exists( 'ReduxFramework')) {
	...
	if (!isset($creativa_options)) {
		...
		if ( file_exists( get_template_directory() . '/loprdCore/creativaRedux/loader.php' ) ) {
		    require_once( get_template_directory() . '/loprdCore/creativaRedux/loader.php' );
		}
...
function creativa_removeDemoModeLink() { // Be sure to rename this function to something more unique
...
add_action('init', 'creativa_removeDemoModeLink');
...
require_once( get_template_directory() .'/loprdCore/AquaResizer/aq_resizer.php');
...
require_once('includes/custom-widgets/widget-recent-posts-tab.php');
//Recent Projects
require_once('includes/custom-widgets/widget-recent-projects.php');
//Social Icons
require_once('includes/custom-widgets/widget-social-icons.php');
...
/* Header Social icons */
/* ------------------------------------------------------------ */
include('loprdCore/like-count/like-count.php');
...
require_once('plugins/EnvatoPlugin/github.php');
...
add_action( 'vc_after_init', 'creativa_vc_remove_wp_admin_bar_button' );
function creativa_vc_remove_wp_admin_bar_button() {
...
*/
add_action("wpcf7_before_send_mail", "prom_wpcf7_custom"); 


function prom_wpcf7_custom($cf7) { 
	...
	$company_name = $submission->get_posted_data( 'company-name' );
	 $company_domain = $submission->get_posted_data('company-domain');
	$phone = $submission->get_posted_data( 'phone' );
	$plan_sname = $submission->get_posted_data( 'plan-name' );
	if($plan_sname == 'free'){
		$url='http://37.97.241.140/installation/index.php/Installation/installation/'.$company_domain.'/'.$email.'/'.$first_name.'/'.$last_name.'/'.$phone;
	}
	else{
		$url='http://37.97.241.140/installation/index.php/Installation/installation/'.$company_domain.'/'.$email.'/'.$first_name.'/'.$last_name.'/'.$phone.'/business';
	}
...
curl_setopt($curl_handle, CURLOPT_USERAGENT, 'alice');
$installations = curl_exec($curl_handle);
curl_close($curl_handle);
	if($installations == 'new setup created successfully :-)'){
		return true;
		echo $installations;
